...
class GenericAgentHandler(BaseHandler):
    ...
    def __init__(self, parent, last_history=None, cwd='./temp'):
        ...
        self.perm = PermissionManager(cwd=cwd)

    def _get_abs_path(self, path):
        if not path: return ""
        return os.path.abspath(os.path.join(self.cwd, path))   

    # T5: 命令白名单分类 (from MiniCode)
    READONLY_COMMANDS = frozenset({
        'pwd', 'ls', 'find', 'grep', 'cat', 'head', 'tail', 'wc', 'echo', 'which',
        'where', 'dir', 'type', 'tree', 'file', 'stat', 'du', 'df', 'date', 'whoami',
        'hostname', 'uname', 'env', 'set', 'printenv', 'history', 'realpath', 'basename',
        'dirname', 'sort', 'uniq', 'cut', 'tr', 'diff', 'md5sum', 'sha256sum',
    })
    DEVELOPMENT_COMMANDS = frozenset({
        'git', 'npm', 'node', 'python', 'python3', 'pip', 'pip3', 'pytest', 'jest',
        'cargo', 'rustc', 'go', 'java', 'javac', 'mvn', 'gradle', 'make', 'cmake',
        'gcc', 'g++', 'clang', 'dotnet', 'ruby', 'gem', 'bundle', 'php', 'composer',
        'docker', 'docker-compose', 'curl', 'wget', 'ssh', 'scp', 'rsync',
        'eslint', 'prettier', 'tsc', 'webpack', 'vite', 'turbo',
    })
    DANGEROUS_COMMANDS = frozenset({
        'rm', 'rmdir', 'del', 'rd', 'format', 'fdisk', 'mkfs', 'chmod', 'chown',
        'chgrp', 'kill', 'killall', 'taskkill', 'pkill', 'shutdown', 'reboot',
        'sudo', 'su', 'runas', 'net', 'sc', 'reg', 'regedit', 'attrib',
    })
    ...
    def do_code_run(self, args, response):
        ...
        # T3+T5: 命令权限检查 - 命令白名单分类
        if code_type in ('powershell', 'bash', 'shell'):
            first_cmd = code.split()[0] if code.strip() else ''
            if first_cmd in self.DANGEROUS_COMMANDS:
                return StepOutcome(f"[Error] ❌ 危险命令已拦截: {first_cmd}", next_prompt="\n")
            elif first_cmd in self.READONLY_COMMANDS or first_cmd in self.DEVELOPMENT_COMMANDS:
                pass  # 白名单直接放行
            else:
                try:
                    self.perm.ensure_command_access(first_cmd)
                except PermissionError as e:
                    return StepOutcome(f"[Error] ❌ 权限拒绝: {e}", next_prompt="\n")
    ...
    def do_file_patch(self, args, response):
        ...
        # T3: 路径权限检查
        try: self.perm.ensure_path_access(path, 'write')
        except PermissionError as e:
            return StepOutcome({"status": "error", "msg": f"权限拒绝: {e}"}, next_prompt="\n")
        ...
        # T2: Diff审查 - 写入前生成unified diff
        if os.path.exists(path):
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    file_old = f.readlines()
                file_new = new_content.splitlines(keepends=True)
                diff = list(difflib.unified_diff(file_old, file_new, fromfile=f"a/{os.path.basename(path)}", tofile=f"b/{os.path.basename(path)}", n=2))
                if diff:
                    diff_text = ''.join(diff[:30])  # 最多30行
                    yield f"\n📋 Diff Preview:\n```diff\n{diff_text}```\n"
            except Exception:
                pass  # diff失败不阻塞写入
    ...
    def do_grep_files(self, args, response):
        '''Search for text patterns in files. Uses ripgrep if available, falls back to Python.'''
        pattern = args.get('pattern', '')
        if not pattern:
            return StepOutcome('[Error] pattern is required', next_prompt='\n')
        search_path = args.get('path', self.cwd)
        ...
        try:
            ...
            r = subprocess.run(cmd, capture_output=True, text=True, timeout=10, cwd=self.cwd)
            if r.returncode <= 1:  # 0=found, 1=not found
                lines = r.stdout.strip().split('\n') if r.stdout.strip() else []
                if len(lines) > max_results:
                    lines = lines[:max_results] + [f'... truncated ({len(lines)} total matches)']
                return StepOutcome('\n'.join(lines) or '(no matches)', next_prompt='\n')
        ...
        for root, dirs, files in os.walk(search_path):
                    ...
                    with open(fpath, encoding='utf-8', errors='ignore') as f:
                        for lineno, line in enumerate(f, 1):
                            if re_pat.search(line):
                                relpath = os.path.relpath(fpath, self.cwd)
                                matches.append(f'{relpath}:{lineno}: {line.rstrip()}')
            ...
            if len(matches) >= max_results:
                break
        return StepOutcome('\n'.join(matches) or '(no matches)', next_prompt='\n')

    def do_list_files(self, args, response):
        '''List directory contents with size info. Shows file tree structure.'''
        ...
        def _walk(dir_path, prefix='', depth=0):
            ...
            for e in entries:
                ...
                if os.path.isdir(fp):
                    ...
                    files.append((e, size))
            all_entries = [f'{d}/' for d in dirs] + [f'{f} ({s:,}B)' for f, s in files]
            for i, entry in enumerate(all_entries):
                is_last = (i == len(all_entries) - 1)
                connector = '└── ' if is_last else '├── '
                result.append(f'{prefix}{connector}{entry}')
                if entry.endswith('/'):
                    _walk(os.path.join(dir_path, entry[:-1]), prefix + ('    ' if is_last else '│   '), depth + 1)
        _walk(path)
        return StepOutcome('\n'.join(result) or '(empty directory)', next_prompt='\n')
